...
| `askTimeoutMs`              | `10000`  | Auto-deny an unanswered permission ask (prompt dialog) after this many milliseconds; the ask settles as a **timeout denial** (`ask_timeout` resolution in logs/review entries) rather than hanging or allowing. `0` disables the timeout — the prompt then waits indefinitely. Applies to both the TUI inline dialog (internal timer) and the RPC/frontend select/input flow (pi's native `{ timeout }` dialog option). **Timeout-retry escalation**: when an ask times out and the agent re-requests the *same operation* (no better no-auth alternative exists), the follow-up ask skips the timeout and waits indefinitely for a human decision — a task that genuinely needs permission must not fail twice by silence. The escalation resets once the human answers the follow-up ask. |
...
- An indirection wrapper — `sudo`, `env`, `xargs`, `time`, `nohup`, `timeout`, `nice`, `parallel`, `rust-parallel`, `rush`, `doas`, `setsid`, `stdbuf`, `watch`, `flock`, or `find`/`fd` carrying a per-result exec flag (`find` with `-exec`/`-execdir`/`-ok`/`-okdir`, `fd` with `-x`/`--exec`/`-X`/`--exec-batch`) — runs a following command that a rule on the wrapper text would otherwise never gate, so its decision is floored the same way (the synthetic `<indirection-bash-wrapper>` pattern in the review log).
  ...
  Tools whose variants can execute, write in place, or dump secrets (`find -exec`/`-delete`, `rg --pre`, `jq --rawfile`/`--slurpfile`, `sed -i`, `yq -i`, `sort -o`, `sudo`/`doas` shells, bare `env`/`printenv`) are excluded from the read-only fast path — a wrapper-wrapped form such as `time rg pattern dir` is floored to `deny` unless its exact form appears in `wrapperAllowlist`.
- Read-only wrapper chains stay allowed: a wrapper chain whose every penetrated inner command is read-only (e.g. `env VAR=x grep pattern`, `timeout 60 cat file`) with no write redirect resolves as `<readonly-chain>` allow. An `env` (or any wrapper) wrapping a non-read-only command still floors to `deny`. Note that a bare `env` or `printenv` (environment dump, with or without a pipe) and `sudo`/`doas` never ride this fast path — they floor to `deny`.
