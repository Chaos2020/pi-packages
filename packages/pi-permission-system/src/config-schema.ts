...
export const unifiedConfigSchema = z
  .strictObject({
    ...
    permissionMode: z
      ...
      .meta({
        description:
          "Feature 6: session permission mode — a coarse layer over per-rule allow/ask/deny. default: unchanged; acceptEdits: auto-allow file-mutation asks; plan: auto-deny file-mutation asks; bypassPermissions: auto-allow every ask. A configured deny is never overridden by any mode.",
        default: "default",
      }),
    ...
    wrapperAllowlist: z.array(z.string().min(1)).optional().meta({
      ...
      markdownDescription:
        'Commands **explicitly trusted** to bypass the indirection-wrapper deny floor.\n\nThe permission system cannot see through indirection wrappers (`sudo`, `env`, `xargs`, `nohup`, `timeout`, …), so a rule that would `allow` such a command is floored to `deny` by default. A command unit keeps its `allow` instead when an entry here matches it as a **complete token-sequence prefix**: split the entry and the command on whitespace; every entry token — the last included — must equal the command token at the same position (whole-token equality, never a substring/prefix inside a longer token). `env PYTHONPATH=` therefore does **not** cover `env PYTHONPATH=/evil sudo rm -rf /`.\n\nOnly add entries you have deliberately vetted; this is the sole escape hatch from the wrapper deny floor.\n\n```json\n"wrapperAllowlist": [\n  "env PYTHONPATH=/opt/lib python3",\n  "timeout 30s npm test"\n]\n```\n\nDefaults to an empty list (every wrapper `allow` is floored to `deny`).',
      default: [],
    }),
