...
import type { Authorizer, AuthorizerVerdict } from "gigapie-permissions";
import {
...
import {
  ...
  reviewOperation,
} from "./command-safety-review";
import type { PromptPermissionDetails } from "gigapie-permissions";
import type { AuthorizerLog } from "gigapie-permissions";
...
function resolveModel(
  ...
  | {
      model: import("./command-safety-review").Model;
      isPrimary: boolean;
    }
  | undefined {
...
function resolveFallback(
  registry: ModelRegistryLike | undefined,
  config: CommandSafetyJudgeConfig,
): import("./command-safety-review").Model | undefined {
  if (!registry) return undefined;
...
function operationContextOf(
  ...
  return {
    ...
    // Upstream replaced the gate's free-text `message` with a structured
    // `payload`; the judge needs a one-line "why was this flagged" for the
    // model, so derive it from the payload's request facts.
    message: describeWhyFlagged(details),
    agentName: details.agentName ?? undefined,
  };
}

/** A one-line "what + why flagged" for the model, from the payload's request facts. */
function describeWhyFlagged(details: PromptPermissionDetails): string {
  const req = details.payload?.request;
  if (req) {
    const why = req.matchedPattern
      ? `${req.surface} matched '${req.matchedPattern}'`
      : req.surface;
    return `${why}: ${req.value}`;
  }
  return (
    details.command ??
    details.path ??
