...
} from "#src/command-safety-config";
import { createCommandSafetyJudge } from "#src/command-safety-judge";
import type {
  CompleteSimpleFn,
  ModelRegistryLike,
} from "#src/command-safety-review";
...
describe("command-safety-judge authorizer", () => {
  ...
  test("edit-tool ask carrying its path: manualConfirmGlobs hit → defer, LLM never called", async () => {
    // Regression (#manual-confirm-path): the tool gate builds the ask's
    // promptDetails with `path` set (forwarded in the pps patch); the judge
    // matches manualConfirmGlobs against details.path. The gate-side wiring
    // (describeToolGate → promptDetails.path) is covered in pi-permission-system
    // tests; here we assert the judge honors the path it receives.
    let called = false;
    const complete = (async () => {
      called = true;
      return { content: [] };
    ...
    const verdict = await judge(
      makeDetails({ path: "/home/lxx/.pi/agent/AGENTS.md" }),
      {} as never,
      log,
    );
